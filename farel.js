...
if (budy.includes("error")){
ano = fs.readFileSync('./sticker/anime/ok.webp')
farel.sendMessage(hehe, ano, sticker, { quoted: ftex})
}
...
${x} *Nama :* ${pushname}
${x} *Nomor :* @${sender.split('@')[0]}
${x} *Seri:* ${serialUser}
${x} *Pengguna:* ${_registered.length}
...
//━━━━━━━━━━━━━━━[ BATAS ]━━━━━━━━━━━━━━━━━//
		    ...
		    case 'ytsearch':
                ...
                *Search Query:* ${input}\n\n`
                ...
                hehe += `───────────────\n
                ${x} *Judul:* ${items[i].title}
                ${x} *ID:* ${items[i].id}
                ${x} *Ditonton:* ${items[i].views}
                ${x} *Durasi:* ${items[i].duration}
                ${x} *Link:* ${items[i].url}\n\n`
                }
            ...
            case "groupinfo":
                ...
                quoted: mek,
                caption: `*「 Group Info 」*\n*${x} Name* : ${groupName}\n*${x} Member* : ${groupMembers.length}\n*${x} Admin* : ${groupAdmins.length}\n*${x} Description* : \n${groupDesc}`,
                });   
                break
            case 'waifu':
            case 'loli':
		    ...
		    case 'status':
				...
				${x} *Total Chat :* ${totalChat.length}
				${x} *Speed :* ${latensi.toFixed(4)} second
				${x} *Runtime :* ${kyun(uptime)}
				${x} *Baterai :* ${baterai.battery}
				${x} *Charged :* ${baterai.isCharge}
				${x} *Mode :* ${publik ? 'public' : 'self'}
				${x} *Prefix :* ${multi ? 'Multi Prefix' : 'No Prefix'}
				${x} *Penggunaan Ram :* ${ram2}
				${x} *Hostname :* ${os.hostname()}
				${x} *Platform :* ${os.platform()}
				${x} *Uptime :* ${kyun(os.uptime())}
				${x} *MNC :* ${mnc}
				${x} *MCC :* ${mcc}
				${x} *Device Model:* ${farel.user.phone.device_model}
				${x} *Device Manufactur :* ${device_manufacturer}
				${x} *Wa Version:* ${farel.user.phone.wa_version}
				${x} *Os Version:* ${farel.user.phone.os_version}
